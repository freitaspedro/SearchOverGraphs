...
def initialize_vertices(g, names, f_values, names_in):
    ...
    for curr_name in names:                 # atualiza os vertices com os valores da feat
        if curr_name in names_in:                   # se o vertice tiver em feat.csv e edges.csv
            v = gt.find_vertex(g, g.vp.name, str(curr_name))
            v = int(v[0])
            g.vp.feat[v] = f_values[i]
            g.vp.rank[v] = 0.0
            g.vp.numt[v] = 0
            g.vp.numn[v] = 0
            cont = cont + 1
...
def calculate_edges(g):
    ...
    for e in g.edges():
        src = e.source()
        tgt = e.target()
        if g.vp.feat[src] != g.vp.feat[tgt]:
            pd = pd + 1
        elif g.vp.feat[src] == 0 and g.vp.feat[tgt] == 0:
            pn = pn + 1
        else:
            pt = pt + 1
...
def main(name, directed, feat_column, budget, runs):
    ...
    print "edges.csv ids - feat.csv ids =", out_feat_in_edges
    ...
    while i < runs:
        ...
        # print "bfs_explored %s" % props.get_v_names(g, bfs_explored)
        ...
        # print "heu_explored %s" % props.get_v_names(g, heu_explored)
...
if __name__ == "__main__":
    main("snap/facebook/1912", False, "119", 40, 60)
    # testando todas as feats para averiguar positivos, pt, pd e pn
    # feat 119 - 63 positives, feat 155 - 108 positives, feat 220 - 586 positives, feat 219 - 222 positives
    # for i in range(0, 480):
        # main("snap/facebook/1912", False, str(i), 340, 60)